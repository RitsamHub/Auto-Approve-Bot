...
class TechifyBots:
    ...
    async def has_joined_channel(
        ...
        return bool(
            doc and channel_id in doc.get("channels", [])
        )
    ...
    async def get_fsub_msg(self, user_id: int):
        ...
        doc = await self.fsub_cache.find_one(
            {"user_id": user_id}
        )
    ...
    async def delete_fsub_msg_db(self, user_id: int):
        ...
        await self.fsub_cache.delete_one(
            {"user_id": user_id}
        )
...
async def check_all_channels_joined(
    bot: Client,
    user_id: int
):
    ...
    for channel_id in AUTH_REQ_CHANNELS:
        ...
        if not await tb.has_joined_channel(
            user_id,
            channel_id
        ):
            return False
...
async def auto_delete_fsub_and_start(
    client: Client,
    user_id: int
):

    if not await check_all_channels_joined(
        client,
        user_id
    ):
        return
    ...
    try:
        ...
        logging.error(
            f"Failed to send start link to {user_id}: {e}"
        )
...
@Client.on_chat_join_request(
    filters.create(is_auth_req_channel)
)
async def join_reqs(
    client: Client,
    ...
    await tb.add_join_req(
        message.from_user.id,
        message.chat.id
    )

    await auto_delete_fsub_and_start(
        client,
        message.from_user.id
    )
...
@Client.on_chat_member_updated(
    filters.chat(AUTH_CHANNELS)
)
async def check_normal_join(
    client: Client,
...
async def del_requests(
    ...
    await message.reply(
        "**⚙ Successfully join request cache deleted.**"
    )
...
async def is_subscribed(
    ...
    expire_at = (
        ...
        + datetime.timedelta(minutes=FSUB_EXPIRE)
        if FSUB_EXPIRE > 0
        else None
    )
    ...
    for channel_id in AUTH_CHANNELS:
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"Bot not admin in auth channel {channel_id}"
                )
...
async def is_req_subscribed(
    ...
    for channel_id in AUTH_REQ_CHANNELS:
        ...
        if await tb.has_joined_channel(
            user_id,
            channel_id
        ):
            continue
        ...
        try:
            ...
            logging.error(
                f"Bot not admin in request channel {channel_id}"
            )
...
async def get_fsub(
    ...
    if AUTH_CHANNELS:
        missing.extend(
            await is_subscribed(bot, user_id)
        )

    if AUTH_REQ_CHANNELS:
        missing.extend(
            await is_req_subscribed(bot, user_id)
        )
...
𝖳𝗈 𝖴𝗌𝖾 𝖳𝗁𝗂𝗌 𝖡𝗈𝗍,
𝖸𝗈𝗎 𝖭𝖾𝖾𝖽 𝖳𝗈 𝖩𝗈𝗂𝗇
𝖠𝖫𝖫 𝖱𝖾𝗊𝗎𝗂𝗋𝖾𝖽 𝖢𝗁𝖺𝗇𝗇𝖾𝗅𝗌.

𝖱𝖾𝗊𝗎𝗂𝗋𝖾𝖽 𝖢𝗁𝖺𝗇𝗇𝖾𝗅𝗌 ({len(missing)})

𝖠𝖿𝗍𝖾𝗋 𝖩𝗈𝗂𝗇𝗂𝗇𝗀,
𝖢𝗅𝗂𝖼𝗄 “𝖳𝗋𝗒 𝖠𝗀𝖺𝗂𝗇” 𝖡𝖾𝗅𝗈𝗐.
</b>
""",
